...
public unsafe class Renderer
{
 ...
 private static byte[]  elements = new byte[24] 
 {
  0, 1,
  1, 2,
  2, 3,
 ...
 public static void Main()
 {
  //Window init
   glfwInit();
   glfwWindowHint(0x22002, 4);       //Context major 4
   glfwWindowHint(0x22003, 5);       //Context minor 5
   glfwWindowHint(0x22006, 1);       //Forward compatible
   glfwWindowHint(0x22008, 0x32001); //Core profile
   fixed (byte* winname = winstr)
   {
    window = glfwCreateWindow(1600, 900, (IntPtr)winname, (IntPtr)0, (IntPtr)0);
   }
  ...
  //Buffer init
   ...
   glBindBuffer(0x8892, bufobjs[0]); //Array buffer
   fixed (float* vertptr = vertices)
   {
    glBufferData(0x8892, 96, (IntPtr)vertptr, 0x88E4); //Static draw
   }
   glBindBuffer(0x8893, bufobjs[1]); //Element buffer
   fixed (byte* elemptr = elements)
   {
    glBufferData(0x8893, 24, (IntPtr)elemptr, 0x88E4);
   }
   glVertexAttribPointer(0, 3, 0x1406, false, 12, (IntPtr)0); //GL_FLOAT
  ...
  //Shader init
   ...
   fixed (byte* vertstrptr = vertshaderstr)
   {
    ...
    glShaderSource(vertshader, 1, (IntPtr)(&ptr), (IntPtr)0);
   }
   glCompileShader(vertshader);
   glGetShaderiv(vertshader, 0x8B81, (IntPtr)(&result)); //Compile status
   if (result == 0)
   {
   ...
   glGetShaderiv(fragshader, 0x8B81, (IntPtr)(&result)); //Compile status
   if (result == 0)
   {
    fixed (byte* bufptr = buffer)
    {
   ...
   glGetProgramiv(shader, 0x8B82, (IntPtr)(&result)); //Link status
   if (result == 0)
   {
    fixed (byte* bufptr = buffer)
    {
